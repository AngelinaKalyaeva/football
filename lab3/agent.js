...
const dtFlags = require('./dt_flags_three'); //Подключение модуля дерева решений для двух игроков
const dtGoalie = require('./dt_goalie'); //Подключение модуля дерева решений для двух игроков
...
class Agent {
    constructor(isGoalie) {
        ...
        this.mgr = {
            getAction(dt, p) {
                dtFlags.commandManager.p = p;
                dtGoalie.commandManager.p = p;
    ...
    processMsg(msg) {  //Обработка сообщения
        ...
        if (data.cmd === "hear" && data.msg.includes("goal")) {
            ...
            dtFlags.DT_FLAGS.state.next = 0;
            dtFlags.DT_FLAGS.state.command = null;
            this.run = false;
        }
    ...
    analyzeEnv(msg, cmd, p) {  //Анализ сообщения
        if (cmd === "see" && this.run && this.isMoved) {
            if (!this.golie) {
                console.log("not golie");
                var action = this.mgr.getAction(dtFlags.DT_FLAGS, p);
                this.socketSend(action.n, action.v);
            } else {
                console.log("golie");
                var action = this.mgr.getAction(dtGoalie.DT, p);
                this.socketSend(action.n, action.v);
            }
    ...
    getAllSeeFlags(p) {
        ...
        for (let i = 1; i < p.length; ++i) {
            if (p[i].cmd.p.length >= 0 &&
                (p[i].cmd.p[0] === "f" || p[i].cmd.p.join("") === "gr" || p[i].cmd.p.join("") === "gl") &&
                p[i].p.length >= 2
            ) {
